...
function doPromiseAll() {
  ...
  Promise.all(operations).then(function success(data) {
    log('Result 1:', data[0].results[0].address_components[0].long_name);
    log('Result 2:', data[1].results[0].address_components[0].long_name);
  }, error);
}
...
function doPromiseChain() {
  ...
  Module.get(op1url).then(function success(data1) {
    ...
    log('Result 2:', data2[0].results[0].address_components[0].long_name);
  }, error);
}
